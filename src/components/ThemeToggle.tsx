...
export function ThemeToggle() {
  ...
  return (
    <div className="flex items-center gap-2">
      <Sun
        className={`h-4 w-4 transition-colors ${isDark ? "text-ink-muted" : "text-accent-secondary"}`}
        aria-hidden="true"
      />
      ...
      >
        <span
          className={`inline-block h-4 w-4 transform rounded-full bg-black/80 shadow-card
            transition-transform hover:border hover:border-ink-muted ${isDark ? "translate-x-6" : "translate-x-1"}`}
        />
      </button>
